					...
					<table width="100%" border="0" cellspacing="0" style="margin-top:30px">
						...
						<tr>
							<td class="listtd">
								...
								&nbsp;&nbsp;&nbsp;&nbsp; 
								<a href="${pageContext.request.contextPath}/LoginOutServlet">用户退出</a>
							</td>
						</tr>
					</table>
				</td>
									...
									<c:forEach items="${orders}" var="order">
										<tr>
											...
											<td class="tableopentd03">
												<a href="${pageContext.request.contextPath}/QueryOrderByIdServlet?id=${order.id}">查看</a>&nbsp;&nbsp;
												<c:if test="${order.paystate==0 }">
													<a href="${pageContext.request.contextPath}/DeleteOrderByIdServlet?id=${order.id}"  onclick="javascript:return o_del()">刪除</a>
												</c:if> 
												<c:if test="${order.paystate!=0 }">
													<a href="${pageContext.request.contextPath}/DeleteOrderByIdServlet?id=${order.id}&type=client" onclick="javascript:return o_del()">刪除</a>
												</c:if>
											</td>
										</tr>
									</c:forEach>
